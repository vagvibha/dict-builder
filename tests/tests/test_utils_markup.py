...
class TestConfigLoading(unittest.TestCase):
    ...
    def test_dictionary_list(self):
        write(self.root / "meta.yaml", "dictionaries:\n  - a\n  - b\n")
        write(self.root / "a/meta.yaml", "name: A\n")
        write(self.root / "b/meta.yaml", "name: B\n")
        self.assertEqual(utils.load_dictionary_list(self.root), ["a", "b"])

    def test_dictionary_list_errors(self):
        cases = {
            "missing list": "other: 1\n",
            "unknown dir": "dictionaries:\n  - nosuch\n",
        ...
        for label, text in cases.items():
            with self.subTest(label):
                ...
                with self.assertRaises(DictionaryBuildError):
                    utils.load_dictionary_list(self.root)

    def test_missing_top_level_meta(self):
        with self.assertRaises(DictionaryBuildError):
            utils.load_dictionary_list(self.root)
